...
class EtuDev_Calendar_Helper
{
    ...
    public function getDateFieldHTML($name)
    {
        ...
        $html = '<input type="text" autocomplete="off" value="' . EtuDev_Util_String::cleanFormOutput(
            $this->value
        ) . '"' . EtuDev_Util_Form::parseAttributesToString($this->attributes) . ' ' . EtuDev_Util_Form::parseClasses(
            $this->classes
        ) . ' />';
        ...
        if ($calendarControl || is_numeric($minDate) || strlen(trim($minDate)) || is_numeric($maxDate) || strlen(
            trim($maxDate) || $minHours
        )
        ) {
            if (!$calendarControl) {
