...
class EtuDev_Calendar_HTMLForm
{
    ...
    public function render()
    {
        ...
        $html = '
		<input type="text" class="text" autocomplete="off" value="' . EtuDev_Util_Date::formatDateOutput(
            $this->value
        ) . '" id="' . $this->id . '_input" name="' . $this->name . '_value" />' . '
        ...
        if ($calendarControl || is_numeric($minDate) || strlen(trim($minDate)) || is_numeric($maxDate) || strlen(trim($maxDate))) {
            ...
            $calscript = $calendarControl->render(false, false);
        } else {
            $calscript = '';
        }

        $html .= '
		<script type="text/javascript">$(document).ready(function() {$("#' . $this->id . '_input").datepicker({ showButtonPanel: true, showOn: "focus", dateFormat: "' . str_replace(
            ' ',
            EtuDev_Util_Date::getFormatInputSeparator(),
            static::$formats[EtuDev_Util_Date::getFormatInput()]
        ) . '", altField: "#' . $this->id . '", altFormat: "yy-mm-dd", changeYear: true, changeMonth: true, onClose: function(dateText, int) { if($("#' . $this->id . '_input").val() == "") { $("#' . $this->id . '").val(""); } } }); ' . $calscript . '}); </script>';
